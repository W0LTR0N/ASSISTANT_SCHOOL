...
def _parse_int(
    ...
    try:
        ...
        logger.warning(
            "Invalid int value for %s='%s', using default=%d",
            env_name,
            raw,
            default,
        )
    ...
    if min_value is not None and value < min_value:
        logger.warning(
            "Value %s=%d below min=%d, using default=%d",
            env_name,
            value,
            min_value,
            default,
    ...
    if max_value is not None and value > max_value:
        logger.warning(
            "Value %s=%d above max=%d, using default=%d",
            env_name,
            value,
            max_value,
            default,
...
def _parse_float(
    ...
    try:
        ...
        logger.warning(
            "Invalid float for %s='%s', using default=%f",
            env_name,
            raw,
            default,
        )
...
PLUSOFON_SIP_PORT = _parse_int(
    "PLUSOFON_SIP_PORT", 5060, min_value=1, max_value=65535
)
PLUSOFON_SIP_USER = os.getenv("PLUSOFON_SIP_USER", "")
PLUSOFON_SIP_PASSWORD = os.getenv("PLUSOFON_SIP_PASSWORD", "")

OUTBOUND_RURI_TEMPLATE = os.getenv(
    "OUTBOUND_RURI_TEMPLATE", "sip:{phone}@{host}"
)
OUTBOUND_FROM_TEMPLATE = os.getenv(
    "OUTBOUND_FROM_TEMPLATE", "<sip:{user}@{host}>"
)

# === RTP ===
RTP_PORT_MIN = _parse_int(
    "RTP_PORT_MIN", 10000, min_value=1024, max_value=65535
)
RTP_PORT_MAX = _parse_int(
    "RTP_PORT_MAX", 10100, min_value=1024, max_value=65535
)

if RTP_PORT_MAX < RTP_PORT_MIN:
    logger.warning(
        "RTP_PORT_MAX (%d) < RTP_PORT_MIN (%d), swapping",
        RTP_PORT_MAX,
        RTP_PORT_MIN,
    )
...
SIP_RATE_LIMIT_PER_IP = _parse_int(
    "SIP_RATE_LIMIT_PER_IP", 10, min_value=1, max_value=1000
)
...
# Поддержка обоих имен переменных прокси
TELEGRAM_PROXY_URL = os.getenv("TELEGRAM_PROXY_URL") or os.getenv(
    "PROXY_URL", ""
)
...
# === VAD ===
SPEECH_RMS_THRESHOLD = _parse_int(
    "SPEECH_RMS_THRESHOLD", 300, min_value=0, max_value=10000
)
SILENCE_TO_END_MS = _parse_int(
    "SILENCE_TO_END_MS", 800, min_value=100, max_value=30000
)
MAX_UTTERANCE_MS = _parse_int(
    "MAX_UTTERANCE_MS", 15000, min_value=1000, max_value=120000
)
VAD_PREROLL_MS = _parse_int("VAD_PREROLL_MS", 200, min_value=0, max_value=2000)

# === Limits ===
MAX_CONCURRENT_CALLS = _parse_int(
    "MAX_CONCURRENT_CALLS", 10, min_value=1, max_value=100
)
MAX_CALL_DURATION = _parse_int(
    "MAX_CALL_DURATION", 1800, min_value=60, max_value=7200
)
MEDIA_IDLE_TIMEOUT = _parse_int(
    "MEDIA_IDLE_TIMEOUT", 60, min_value=10, max_value=300
)
MAX_AUDIO_BUFFER_BYTES = _parse_int(
    "MAX_AUDIO_BUFFER_BYTES", 160000, min_value=10000, max_value=1000000
)
MAX_RTP_BUFFER_BYTES = _parse_int(
    "MAX_RTP_BUFFER_BYTES", 320000, min_value=10000, max_value=2000000
)
...
SIP_MAX_RETRANSMITS = _parse_int(
    "SIP_MAX_RETRANSMITS", 7, min_value=1, max_value=15
)
...
DELIVERY_TIMEOUT = _parse_float(
    "DELIVERY_TIMEOUT", 10.0, min_value=1.0, max_value=60.0
)

# === Delivery Retry ===
DELIVERY_RETRY_INTERVAL = _parse_int(
    "DELIVERY_RETRY_INTERVAL", 60, min_value=10, max_value=3600
)
DELIVERY_MAX_RETRIES = _parse_int(
    "DELIVERY_MAX_RETRIES", 5, min_value=1, max_value=20
)

# === Heartbeat ===
HEARTBEAT_INTERVAL = _parse_int(
    "HEARTBEAT_INTERVAL", 30, min_value=5, max_value=600
)
HEARTBEAT_FILE = os.getenv("HEARTBEAT_FILE", "/app/data/heartbeat.txt")

# === Development ===
DEVELOPMENT_MODE = (
    os.getenv("DEVELOPMENT_MODE", "false").strip().lower() == "true"
)
...
def validate_config() -> bool:
    ...
    if not YANDEX_API_KEY or not YANDEX_FOLDER_ID:
        warnings.append(
            "YANDEX_API_KEY / YANDEX_FOLDER_ID не настроены — STT/GPT не будут"
            " работать"
        )
    if not GENVOICE_API_KEY or not GENVOICE_REALTIME_URL:
        warnings.append(
            "GENVOICE_API_KEY / GENVOICE_REALTIME_URL не настроены — TTS не"
            " будет работать"
        )
    ...
    if not TRUSTED_SIP_IPS and not DEVELOPMENT_MODE:
        errors.append(
            "CRITICAL: TRUSTED_SIP_IPS пуст в production — небезопасно"
        )
    if not TELEGRAM_ADMIN_IDS:
        warnings.append(
            "TELEGRAM_ADMIN_IDS пуст — команда /call будет недоступна"
        )
